		...
		if ( is_shop() ) {
			...
			if( !empty( $content ) ) {
				if( !has_shortcode( $content, 'vc_row') ) {
					...
					echo '<div class="grve-container">' . do_shortcode( $content )  . '</div>';
				} else {
					$content = apply_filters( 'the_content', $content );
...
function blade_grve_woo_archive_thumbnail_size ( $size ) {
	return 'shop_catalog';
}
add_filter( 'single_product_archive_thumbnail_size', 'blade_grve_woo_archive_thumbnail_size' );

function blade_grve_woo_meta_sku() {
	wc_get_template( 'single-product/grve-sku-meta.php' );
}
add_action( 'woocommerce_single_product_summary', 'blade_grve_woo_meta_sku', 40 );
