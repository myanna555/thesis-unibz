			...
			<div class="grve-content grve-align-<?php echo esc_attr( $content_position ); ?>" data-animation="<?php echo esc_attr( $content_animation ); ?>">
				...
				<?php if( 'post' == $mode ) { ?>
						<ul class="<?php echo esc_attr( $grve_subheading_classes ); ?>">
							...
							<?php } ?>
							<?php if ( blade_grve_visibility( 'post_date_visibility', '1' ) ) { ?>
				...
				foreach ( $header_menu_options as $key => $value ) {
					if( !empty( $value ) && '0' != $value && blade_grve_check_header_elements_visibility( $key ) ) {
						...
						<?php
							...
							$login_custom_link_url = blade_grve_option( 'login_custom_link_url' );
							if ( !empty( $login_custom_link_url ) ) {
						?>
							<li class="grve-header-element"><a href="<?php echo esc_url( $login_custom_link_url ); ?>"><span class="grve-item"><i class="fa fa-user"></i></span></a></li>
						<?php
							}
						} else if ( 'form' == $key ) {
						?>
...
function blade_grve_print_header_elements_responsive() {
	...
	do_action( 'blade_grve_header_elements_responsive_first_item' );
...
<?php
	...
	do_action( 'blade_grve_header_elements_responsive_last_item' );
 ...
 */
function blade_grve_print_login_responsive_button() {

	$login_custom_link_url = blade_grve_option( 'login_custom_link_url' );

	if( blade_grve_check_header_elements_visibility( 'login' ) && !empty( $login_custom_link_url ) ) {
?>
