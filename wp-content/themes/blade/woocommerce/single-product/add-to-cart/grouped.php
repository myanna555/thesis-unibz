		...
		<tbody>
			...
			if ( version_compare( WC_VERSION, '3.0', '<' ) ) {
				...
				foreach ( $grouped_products as $grouped_product ) {
					...
					echo '<tr id="product-' . esc_attr( get_the_ID() ) . '" class="grve-border woocommerce-grouped-product-list-item ' . esc_attr( implode( ' ', get_post_class() ) ) . '">';
					...
					foreach ( $grouped_product_columns as $column_id ) {
						...
						switch ( $column_id ) {
							...
							case 'label':
								...
								$value .= $grouped_product->is_visible() ? '<a class="grve-subtitle grve-text-content grve-text-hover-primary-1" href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', get_permalink( $grouped_product->get_id() ), $grouped_product->get_id() ) ) . '">' . $grouped_product->get_name() . '</a>' : $grouped_product->get_name();
								$value .= '</label>';
								break;
							case 'price':
